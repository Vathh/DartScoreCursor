...
use App\Repositories\QuickGameSessionRepository;
...
class QuickGameSessionService
{
     ...
     */
    public function createSession(int $lobbyId, int $hostUserId, string $scoringMode, string $gameType, int $legsCount, int $playersCount): \App\Models\QuickGameSession
    {
    ...
    public function getState(int $sessionId, ?int $currentUserId): array
    {
        ...
        $players = $session->lobby->players->map(function ($p) {
            return [
                'id' => $p->id,
        ...
        if ($currentUserId !== null) {
            foreach ($session->lobby->players as $i => $lp) {
